...
import "../styles/Game.css"; // untuk animasi dan efek visual
...
const ArenaPVP = () => {
  ...
  useEffect(() => {
    ...
    const fetchStatus = async () => {
      const { status, opponentStatus, isMyTurn, gameOver, winner } = await getPVPGameState(contract, account);
      setStatus(status);
  ...
  if (!account) {
    return <div className="text-center mt-10">Connect wallet to continue.</div>;
  }

  if (!status || !opponentStatus) {
    return <div className="text-center mt-10">Loading game status...</div>;
  }
